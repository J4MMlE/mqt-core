...
# Run all MLIR unit tests (ctest filter, faster than full suite)
test-mlir config=default_config: (_ensure_configured config) (_build "all" config)
    ctest --test-dir {{build_dir}}/{{config}} -C {{config}} -j {{jobs}} --output-on-failure -L "mqt-mlir-unittests"

# Run only the QCO quaternion merge unit tests (fastest iteration)
