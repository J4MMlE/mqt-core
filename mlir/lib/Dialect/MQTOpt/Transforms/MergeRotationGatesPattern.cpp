...
struct MergeRotationGatesPattern final
     ...
     */
    [[nodiscard]] static bool areUsersUnique(
        const mlir::ResultRange::user_range& users) {
        return llvm::none_of(
            users, [&](auto* user) { return user != *users.begin(); });
    }
    ...
    static UnitaryInterface createOpAdditiveAngle(
        UnitaryInterface op,
        UnitaryInterface user,
        ...
        auto add =
            rewriter.create<mlir::arith::AddFOp>(loc, opParam, userParam);
    ...
    static Quaternion createAxisQuaternion(mlir::Value angle,
                                           char axis,
                                           mlir::Location loc,
                                           mlir::PatternRewriter& rewriter) {
    ...
    static Quaternion hamiltonProduct(Quaternion q1,
                                      Quaternion q2,
                                      UnitaryInterface op,
                                      mlir::PatternRewriter& rewriter) {
        auto loc = op->getLoc();
    ...
    static UnitaryInterface uGateFromQuaternion(
        Quaternion q,
        UnitaryInterface op,
        mlir::PatternRewriter& rewriter) {
        auto loc = op->getLoc();
        ...
        auto thetaMinus =
            rewriter.create<mlir::math::Atan2Op>(loc, xMinus, q.y);
     ...
     * rotation gates into quaternions. These quaterions are then
     ...
     * @return A new rotation gate.
     */
    static UnitaryInterface createOpQuaternionMergedAngle(
        UnitaryInterface op,
        UnitaryInterface user,
        mlir::PatternRewriter& rewriter) {
